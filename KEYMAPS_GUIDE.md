...
| `<C-h>` `<C-j>` `<C-k>` `<C-l>` | Перейти между окнами без выхода из Insert Mode | `n` `i` |
...
| `<leader>gl` | Открыть полноразмерный граф истории Git |

В графе LazyGit `+` разворачивает активную панель, `<C-l>` открывает параметры
журнала (включая граф всех веток), а `<Enter>` показывает файлы выбранного коммита.
...
> Сначала используется Bing, при ошибке автоматически выполняется повтор через Google.
